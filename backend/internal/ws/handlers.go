	...
	if existingRoom != nil {
		...
		if isReconnection, _ := stateManager.CheckSessionInRoom(roomCode, sessionID); isReconnection {
			if snapshot := stateManager.GetRoomState(roomCode); snapshot != nil {
				// Validate game mode matches before restoring
				if snapshot.GameState != nil && snapshot.GameState.GameMode != "" && snapshot.GameState.GameMode != gameMode {
					log.Printf("Skipping state restore: snapshot mode %s != requested mode %s", snapshot.GameState.GameMode, gameMode)
				} else {
